...
def generate_subgraph_datasets(params, splits=['train', 'valid'], saved_relation2id=None, saved_attribute2id=None, max_label_value=None):

    testing = 'test' in splits
    adj_list, triplets, neg_triplets, numerical_literals, entity2id, relation2id, attribute2id, id2entity, id2relation, id2attribute = process_files(params.file_paths, saved_relation2id, saved_attribute2id)
    ...
    data_path = os.path.join(params.main_dir, f'data/{params.dataset}/attribute2id.json')
    if not os.path.isdir(data_path) and not testing:
        with open(data_path, 'w') as f:
            json.dump(attribute2id, f)
    
    graphs = {}

    for split_name in splits:
        graphs[split_name] = {'triplets': triplets[split_name], 'neg': neg_triplets[split_name], 'max_size': params.max_links}
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None, include_attribute=None, add_traspose_rels=False, num_neg_samples_per_link=1, use_kge_embeddings=False, dataset='', kge_model='', file_name='', use_numeric = False):
        ...
        ssp_graph, pos_triples, neg_triples, literals, entity2id, relation2id, attribute2id, id2entity, id2relation, id2attribute = process_files(raw_data_paths, included_relations, include_attribute)
        self.num_rels = len(ssp_graph)
        self.num_atts = len(attribute2id)
        ...
        self.id2attribute = id2attribute
        self.literals = literals
        self.pos_triples = pos_triples['train']
        self.neg_triples = neg_triples['train']
        #self.missing_mask = np.ones_like(self.literals)
    ...
    def attribute_normalization(self, density, main_dir = None, experiment_name = None, saved_max_lit = None, saved_min_lit = None, saved_missing_mask = None):

        missing = np.where(np.random.uniform(0, 1, size=[self.literals.shape[0], self.literals.shape[1]]) >= density, 0, 1)

        self.missing_mask = missing if saved_missing_mask is None else saved_missing_mask
        missing = self.missing_mask
        zero_indices = np.where(missing == 0)
        self.literals[zero_indices] = np.nan
        max_lit = np.nanmax(self.literals, axis=0) if saved_max_lit is None else saved_max_lit
        min_lit = np.nanmin(self.literals, axis=0) if saved_min_lit is None else saved_min_lit

        if self.file_name == 'train':
            min_max_lit = {'max_lit': max_lit, 'min_lit': min_lit}
            exps_dir = os.path.join(main_dir, 'experiments')
            exp_dir = os.path.join(exps_dir, experiment_name)
            with open(os.path.join(exp_dir, 'min_max_lit.npy'), 'wb') as f:
                    np.save(f, min_max_lit)
            with open(os.path.join(exp_dir, 'missing_mask.npy'), 'wb') as f:
                    np.save(f, missing)
        self.literals = (self.literals - min_lit) / (max_lit - min_lit + 1e-8)
        self.literals[zero_indices] = 0
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        ...
        if self.use_numeric:
            ...
            subgraph.ndata['attribute'] = torch.FloatTensor(self.literals[subgraph.ndata['parent_id']])
            subgraph.ndata['mask'] = torch.tensor(self.missing_mask[subgraph.ndata['parent_id']], dtype = torch.long)
        else:
            pass
    ...
    def _prepare_features_new(self, subgraph, n_labels, r_labels, n_feats=None):
        # One hot encode the node label feature and concat to n_featsure
        n_nodes = subgraph.number_of_nodes()
        ...
        subgraph.ndata['feat'] = torch.FloatTensor(n_feats)
        #print(subgraph.ndata['feat'].size())
        ...
        subgraph.ndata['r_label'] = torch.LongTensor(np.ones(n_nodes) * r_labels)
        self.n_feat_dim = n_feats.shape[1]  # Find cleaner way to do this -- i.e. set the n_feat_dim
        return subgraph
