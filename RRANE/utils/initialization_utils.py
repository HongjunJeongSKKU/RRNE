...
def initialize_experiment(params, file_name):
    ...
    elif file_name == 'test_isolated.py':
        params.test_exp_dir = os.path.join(params.exp_dir, f"test_{params.dataset}_{params.constrained_neg_prob}")
        if not os.path.exists(params.test_exp_dir):
            os.makedirs(params.test_exp_dir)
        file_handler = logging.FileHandler(os.path.join(params.test_exp_dir, f"log_test_isolated.txt"))
    elif file_name == 'visualization_subgraph_repr.py':
        params.test_exp_dir = os.path.join(params.exp_dir, f"test_{params.dataset}_{params.constrained_neg_prob}")
        if not os.path.exists(params.test_exp_dir):
            os.makedirs(params.test_exp_dir)
        file_handler = logging.FileHandler(os.path.join(params.test_exp_dir, f"log_test_visualize.txt"))
    else:
...
def initialize_model(params, model, load_model=False):
    ...
    if load_model and os.path.exists(os.path.join(params.exp_dir, 'best_graph_classifier.pth')):
        ...
        attribute2id_path = os.path.join(params.main_dir, f'data/{params.dataset}/attribute2id.json')
        with open(attribute2id_path) as f:
            attribute2id = json.load(f)


        logging.info('No existing model found. Initializing new model..')
        graph_classifier = model(params, relation2id, attribute2id).to(device=params.device)
