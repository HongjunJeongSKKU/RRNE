...
def main(params):
    ...
    for r in range(1, params.runs + 1):
        ...
        generate_subgraph_datasets(params, splits=['test'],
                                   ...
                                   saved_attribute2id=graph_classifier.attribute2id,
                                   max_label_value=graph_classifier.gnn.max_label_value)

        test = SubgraphDataset(params.db_path, 'test_pos', 'test_neg', params.file_paths, graph_classifier.relation2id, graph_classifier.attribute2id,
                               add_traspose_rels=params.add_traspose_rels,
                               num_neg_samples_per_link=params.num_neg_samples_per_link,
                               use_kge_embeddings=params.use_kge_embeddings, dataset=params.dataset,
                               kge_model=params.kge_model, file_name=params.test_file, use_numeric = params.use_numeric)

        if params.use_numeric:
            ...
            test.attribute_normalization(density = params.attribute_density, saved_max_lit = min_max_lit['max_lit'], saved_min_lit = min_max_lit['min_lit'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_neg_file", "-tnf", type=str, default="train_neg",
                        help="Name of file containing training triplets")
    parser.add_argument("--test_neg_file", "-testnf", type=str, default="test_neg",
                        help="Name of file containing test triplets")
    ...
    params.file_paths = {
        ...
        'test_neg': os.path.join(params.main_dir, 'data/{}/{}.txt'.format(params.dataset, params.test_neg_file)),
